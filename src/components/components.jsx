...
import { AlertTriangle, BookOpen, Clock, User, X } from 'lucide-react';
...
export const ErrorState = ({ title = 'No se pudo cargar la informacion', message, onRetry }) => (
  <div className="empty-state" style={{ background: COLORS.surface, border: `1px solid ${COLORS.border}`, borderRadius: 12 }}>
    <div style={{ display: 'flex', justifyContent: 'center', marginBottom: 12 }}>
      <AlertTriangle size={44} color={COLORS.danger} />
    </div>
    <p style={{ marginTop: 12, fontWeight: 700, color: COLORS.textPrimary }}>{title}</p>
    {message && <p style={{ fontSize: 13, marginTop: 6, color: COLORS.textSecondary }}>{message}</p>}
    {onRetry && (
...
export const CourseCard = ({ curso, categorias = [], onEnroll, onDetail, inscrito = false, progress = null }) => {
  ...
  const pct = curso.max > 0 ? Math.min(100, Math.round((curso.inscritos / curso.max) * 100)) : 0;
  return (
    <div
      className="card"
